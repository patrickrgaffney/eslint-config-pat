...
# List of peer and optional dependencies from package.json
# in the following format: "package-name@semver".
DEPS=$(jq -rc '.peerDependencies | to_entries | map("\(.key)@\(.value|tostring|split(">=")|last)")|.[]' < package.json | tr '\n' ' ')
DEPS+=$(jq -rc '.optionalDependencies | to_entries | map("\(.key)@\(.value|tostring|split(">=")|last)")|.[]' < package.json | tr '\n' ' ')
